...
function ethereumTargetDifficulty (target) {
    let targetHex = normalizeHex(target || '');
    if (!targetHex) return 0;
    let targetValue = BigInt('0x' + targetHex);
    if (targetValue === 0n) return 0;
    let diff1 = (1n << 256n) - 1n;
    return (diff1 / targetValue).toString();
}

function ethereumDifficultyValue (difficulty) {
    if (difficulty === undefined || difficulty === null) return null;
    if (typeof difficulty === 'string' && difficulty.indexOf('0x') === 0) return BigInt(difficulty).toString();
    return difficulty.toString();
}

function ethereumWorkValue (work, index, keys, fallback) {
    if (Array.isArray(work)) return work[index] || fallback;
    if (!work || typeof work !== 'object') return fallback;
    for (let i = 0; i < keys.length; i++) {
        if (work[keys[i]] !== undefined && work[keys[i]] !== null) return work[keys[i]];
    }
    return fallback;
}
...
function normalizeEthereumResult (method, result) {
    switch (method) {
        ...
        case 'eth_getWork':
            let work = result || [];
            let powHash = ethereumWorkValue(work, 0, ['pow_hash', 'powHash', 'blocktemplate_blob', 'header_hash', 'headerHash'], '');
            let seedHash = ethereumWorkValue(work, 1, ['seed_hash', 'seedHash'], '');
            let target = ethereumWorkValue(work, 2, ['target', 'target_hash', 'targetHash'], '');
            let difficulty = ethereumWorkValue(work, 3, ['difficulty'], null);
            return {
                blocktemplate_blob: normalizeHex(ethereumWorkValue(work, 4, ['blocktemplate_blob', 'blob'], powHash)),
                difficulty: difficulty !== null ? ethereumDifficultyValue(difficulty) : ethereumTargetDifficulty(target),
                height: fromRpcQuantity(ethereumWorkValue(work, 5, ['height', 'number'], 0)),
                num_transactions: fromRpcQuantity(ethereumWorkValue(work, 6, ['num_transactions', 'numTransactions'], 0)),
                pow_hash: normalizeHex(powHash),
                reserved_offset: fromRpcQuantity(ethereumWorkValue(work, 7, ['reserved_offset', 'reserveOffset'], 0)),
                seed_hash: normalizeHex(seedHash),
                status: 'OK',
...
function isEthereumDaemon () {
    return config.daemonType && config.daemonType.toLowerCase() === 'ethereum';
}
        ...
        var req = (port === 443 ? https : http)
            .request(options, function (res) {
                ...
                res.on('end', function () {
                    ...
                    if (isEthereumDaemon() && replyJson && !replyJson.error) {
...
function rpc(host, port, method, params, callback) {
    ...
    var payload = isEthereumDaemon() ? ethereumRpcPayload(method, params) : { method: method, params: params };
    var request = {
        id: "0",
        jsonrpc: "2.0",
    ...
    jsonHttpRequest(host, port, data, function (error, replyJson) {
        ...
        if (isEthereumDaemon() && !(result && (result.block_header || result.blocktemplate_blob || result.status || result.height !== undefined))) {
...
function batchRpc(host, port, array, callback) {
    ...
    for (var i = 0; i < array.length; i++) {
        var payload = isEthereumDaemon() ? ethereumRpcPayload(array[i][0], array[i][1]) : { method: array[i][0], params: array[i][1] };
        rpcArray.push({
            id: i.toString(),
            jsonrpc: "2.0",
	    password: config.wallet.password,
