...
config.daemonType = config.daemonType ? config.daemonType.toLowerCase() : 'default';
let isEthereumDaemon = config.daemonType === 'ethereum';

// Set RandomX algorithm
const randomXAlgorithm = "randomx";
if (!isEthereumDaemon && config.cnAlgorithm && config.cnAlgorithm !== randomXAlgorithm) {
	log('warn', logSystem, 'Ignoring configured algorithm %s; this pool is RandomX-only', [config.cnAlgorithm]);
}
if (!isEthereumDaemon) {
	config.cnAlgorithm = randomXAlgorithm;
	config.isRandomX = true;
}

let cnAlgorithm = isEthereumDaemon ? (config.cnAlgorithm || 'ethereum') : randomXAlgorithm;
...
if (!isEthereumDaemon) {
	if (!cnHashing || !cnHashing[cnAlgorithm]) {
		log('error', logSystem, 'Invalid RandomX algorithm: %s', [cnAlgorithm]);
	} else {
	...
	switch (message.type) {
		...
		case 'BlockTemplate':
			try {
				let new_hash;
				if (isEthereumDaemon) {
					new_hash = Buffer.from(message.block.pow_hash || message.block.blocktemplate_blob || '', 'hex');
				} else {
					let buffer = Buffer.from(message.block.blocktemplate_blob, 'hex');
					new_hash = Buffer.alloc(32);
...
function BlockTemplate (template, parent, indexOfChildPool) {
	...
	this.blocktemplate_blob = template.blocktemplate_blob || template.pow_hash;
	...
	this.extraNonce = 0;
	if (isEthereumDaemon) {
		this.isRandomX = config.isRandomX === true || !!template.seed_hash;
		this.seed_hash = template.seed_hash;
		this.next_seed_hash = template.next_seed_hash;
		this.reserveOffset = template.reserveOffset || template.reserved_offset || 0;
		this.clientNonceLocation = this.reserveOffset;
		this.prev_hash = Buffer.alloc(32);
		this.buffer.copy(this.prev_hash, 0, 0, Math.min(this.buffer.length, 32));
		return;
	}
...
function getShareBuffer (miner, job, blockTemplate, params) {
	...
	if (isEthereumDaemon) {
		return Buffer.from(blockTemplate.powHash || blockTemplate.blocktemplate_blob, 'hex');
	}
...
function processShare (miner, job, blockTemplate, params) {
	...
	if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability) {
		...
		if (!blockTemplate.isRandomX || !blockTemplate.seed_hash) {
			log('error', logSystem, 'Rejected non-RandomX block template at height %d', [blockTemplate.height]);
			return false;
		}
	...
	if (!isEthereumDaemon && hash.toString('hex') !== resultHash) {
	...
	if (hashDiff.ge(blockTemplate.difficulty)) {
		...
		apiInterfaces.rpcDaemon(isEthereumDaemon ? 'eth_submitWork' : 'submitblock', isEthereumDaemon ? ['0x' + params.nonce, '0x' + (blockTemplate.powHash || blockTemplate.blocktemplate_blob), '0x' + resultHash] : [shareBuffer.toString('hex')], function (error, result) {
			if (error) {
			    ...
			    let blockFastHash = isEthereumDaemon ? resultHash : utils.cnUtil.get_block_id(shareBuffer, cnBlobType).toString('hex');
				log('info', logSystem,
			...
			if (hashDiff.ge(childBlockTemplate.difficulty)) {
				...
				if (mergedBuffer === null) {
					...
					let onChildSuccess = (result) => {
						let blockFastHash = isEthereumDaemon ? resultHash : utils.cnUtil.get_block_id(mergedBuffer, 2).toString('hex')
						log('info', logSystem,
					...
					apiInterfaces.rpcDaemon(isEthereumDaemon ? 'eth_submitWork' : 'submitblock', isEthereumDaemon ? ['0x' + params.nonce, '0x' + (childBlockTemplate.powHash || childBlockTemplate.blocktemplate_blob), '0x' + resultHash] : [mergedBuffer.toString('hex')], function (error, result) {
						if (error) {
