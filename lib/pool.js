...
if (config.cnAlgorithm && config.cnAlgorithm !== randomXAlgorithm) {
...
let cnAlgorithm = randomXAlgorithm;
...
function BlockTemplate (template, parent, indexOfChildPool) {
	...
	this.powHash = template.pow_hash;
...
function processShare (miner, job, blockTemplate, params) {
	...
	if (hashDiff.ge(blockTemplate.difficulty)) {
		...
		apiInterfaces.rpcDaemon('eth_submitWork', ['0x' + params.nonce, '0x' + (blockTemplate.powHash || blockTemplate.blocktemplate_blob), '0x' + resultHash], function (error, result) {
			if (error) {
		...
		if (mergedMining) {
			...
			if (hashDiff.ge(childBlockTemplate.difficulty)) {
				...
				if (mergedBuffer === null) {
					...
					apiInterfaces.rpcDaemon('eth_submitWork', ['0x' + params.nonce, '0x' + (childBlockTemplate.powHash || childBlockTemplate.blocktemplate_blob), '0x' + resultHash], function (error, result) {
						if (error) {
