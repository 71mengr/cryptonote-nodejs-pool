	...
	async.waterfall([
		...
		function (blocks, callback) {
			async.filter(blocks, function (block, mapCback) {
				...
				let rpcMethod = 'eth_getBlockByNumber';
				apiInterfaces.rpcDaemon(rpcMethod, [blockHeight, config.blockUnlocker.useFirstVout], function (error, result) {
					if (error) {
					...
					if (!result.block_header) {
						log('error', logSystem, 'Error with eth_getBlockByNumber RPC request for block %s - %j', [block.serialized, error]);
						block.unlocked = false;
						mapCback();
						return;
					}
