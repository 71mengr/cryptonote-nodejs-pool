...
function collectStats () {
    ...
    apiInterfaces.rpcDaemon('eth_getBlockByNumber', ['latest', false], function (error, reply) {
        if (error) {
            log('error', logSystem, 'Error getting last block data %j', [error]);
            callback(true);
            return;
        }
        // reply is the raw block from Geth
        if (!reply || !reply.number) {
            log('error', logSystem, 'Invalid block response: %j', [reply]);
            callback(true);
            return;
        }
        
        let blockNumber = parseInt(reply.number, 16);
        let blockHash = reply.hash;
        let blockTimestamp = parseInt(reply.timestamp, 16);
        let blockDifficulty = parseInt(reply.difficulty, 16);
		...
		network: function (callback) {
			let networkDataRpcMode = 'eth_blockNumber';

function getNetworkData(callback, rpcMode) {
    if (!rpcMode) rpcMode = networkDataRpcMode;

    if (rpcMode === 'eth_blockNumber') {
        apiInterfaces.rpcDaemon('eth_blockNumber', [], function (error, reply) {
            if (error || reply === undefined) {
                getNetworkData(callback, 'eth_getBlockByNumber');
                return;
            } else {
                networkDataRpcMode = 'eth_blockNumber';
                let height = parseInt(reply, 16);
                callback(null, {
                    difficulty: 0,
                    height: height
                });
        ...
        apiInterfaces.rpcDaemon('eth_getBlockByNumber', ['latest', false], function (error, reply) {
            if (error) {
                ...
                networkDataRpcMode = 'eth_getBlockByNumber';
                let blockNumber = parseInt(reply.number, 16);
                let blockDifficulty = parseInt(reply.difficulty, 16);
                callback(null, {
                    difficulty: blockDifficulty,
		...
		config: function (callback) {
			callback(null, {
				...
				cnAlgorithm: config.cnAlgorithm || 'randomx',
...
function getNetworkData (callback, rpcMode) {
	...
	// Try eth_blockNumber RPC method first if available
	if (rpcMode === 'eth_blockNumber') {
		apiInterfaces.rpcDaemon('eth_blockNumber', [], function (error, reply) {
			if (error || !reply) {
				getNetworkData(callback, 'eth_getBlockByNumber');
	...
	// Else fallback to eth_getBlockByNumber
	else {
		apiInterfaces.rpcDaemon('eth_getBlockByNumber', ['latest', false], function (error, reply) {
			if (error) {
				log('error', logSystem, 'Error getting network data %j', [error]);
...
function getLastBlockData (callback) {
	apiInterfaces.rpcDaemon('eth_getBlockByNumber', ['latest', false], function (error, reply) {
		...
		if (config.blockUnlocker.useFirstVout) {
			apiInterfaces.rpcDaemon('eth_getBlockByNumber', [blockHeader.height, true], function (error, result) {
				if (error) {
