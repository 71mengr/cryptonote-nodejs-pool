...
function parseAddressPrefix(addressPrefix, encoding) {
    if (addressPrefix === undefined || addressPrefix === null) return null;
    if (typeof addressPrefix === 'number') return parseInt(addressPrefix);
    if (Buffer.isBuffer(addressPrefix)) return parseInt(addressPrefix.toString(encoding));
    return parseInt(addressPrefix.toString());
}

function decodeAddressPrefix(address) {
    if (!address) return null;

    return parseAddressPrefix(cnUtil.address_decode(Buffer.from(address)));
}

/**
 * Validate miner address
 **/
var addressBase58Prefix = config.poolServer.pubAddressPrefix ? parseInt(config.poolServer.pubAddressPrefix) : decodeAddressPrefix(config.poolServer.poolAddress);
let integratedAddressBase58Prefix = config.poolServer.intAddressPrefix ? parseInt(config.poolServer.intAddressPrefix) : (addressBase58Prefix === null ? null : addressBase58Prefix + 1);
...
function getAddressPrefix(address) {
    ...
    let addressPrefix = parseAddressPrefix(cnUtil.address_decode(addressBuffer), 'hex');
    if (!addressPrefix && cnUtil.address_decode_integrated) {
        addressPrefix = parseAddressPrefix(cnUtil.address_decode_integrated(addressBuffer));
    }
...
exports.validateMinerAddress = function(address) {
    ...
    if (addressBase58Prefix !== null && addressPrefix === addressBase58Prefix) return true;
...
exports.validateChildMinerAddress = (address, index) => {
	let childAddressBase58Prefix = decodeAddressPrefix(config.childPools[index].poolAddress);
	let childIntegratedAddressBase58Prefix = config.poolServer.intChildAddressPrefix ? parseInt(config.childPools[index].intAddressPrefix) : (childAddressBase58Prefix === null ? null : childAddressBase58Prefix + 1);

	let addressPrefix = getAddressPrefix(address);
	if (childAddressBase58Prefix !== null && addressPrefix === childAddressBase58Prefix) return true;
	else if (childIntegratedAddressBase58Prefix !== null && addressPrefix === childIntegratedAddressBase58Prefix) return true;
	return false;
}
